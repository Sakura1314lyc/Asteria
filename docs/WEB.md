...
- 文献入口：跨项目搜索论文记录。
- 项目概览：研究问题、协议、统计、运行历史，以及方案—检索—筛选—证据—报告证据脊柱。
...
本地工作台支持编辑项目名称、主题、研究问题、工作流类型和输出语言；字段变更会写入 `project_events`，保留字段级 before/after 轨迹。模型连接也可修改，API Key 留空时继续使用原密钥。
...
- `ui:rehearse` 只验证演示所需的真实选择器，不移动鼠标、不修改数据。
